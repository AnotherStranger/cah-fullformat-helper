...
bool CardsDeck::getSort() const
{
    return sort;
}

void CardsDeck::setSort(bool value)
{
    sort = value;
}
...
void CardsDeck::sortCards(){
    if(!sorted&&sort){
        struct {
            bool operator()(const QSharedPointer<Card> &s1, const QSharedPointer<Card> &s2) const
            {
                return QString::compare(s1->toLatex(), s2->toLatex(), Qt::CaseInsensitive) <0 ;
            }
        } customLess;
...
bool isPossibleDuplicate(QSharedPointer<Card> card, QSharedPointer<Card> other, double threshold){
   ...
   // Don't calculate the Levenshtein distance when the min is already greater than the threshold.
   if(minLevenshtein <= dupThreshold){
       isDuplicate = levenshtein_distance(card->getText().toStdString(), other->getText().toStdString()) <= dupThreshold;
   }
