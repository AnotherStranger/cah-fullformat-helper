...
void MainClass::run()
{
    ...
    if(cmdParser.isSet("duplicates")){
        for(auto dup : deck.getPossibleDuplicates(cmdParser.value("duplicates").toDouble())){
...
void MainClass::setupCmdParser(){
    cmdParser.setApplicationDescription("An application to help you managing your own CAH-Game.");
    ...
    {{"d","duplicates"},
     "Prints possible Duplicated Cards in CSV. Each pair is seperated by an empty line.",
     "Tolerance in percent."}
                });
    cmdParser.addPositionalArgument("files", "Files to read. Possible formats are .csv and .tex");
}
