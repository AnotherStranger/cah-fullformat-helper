...
BlackCard::BlackCard(QString text, QString category, int cardCount, QObject *parent) : Card(text,category,parent){
...
QString BlackCard::toLatex(){
    ...
    return latex.replace("<cardCount>", QString::number(getCardCount())).replace("<category>",getCategory()).replace("<text>",getText());
}
