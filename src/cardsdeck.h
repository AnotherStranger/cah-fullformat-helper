 ...
 * This type was defined for convenience and is used to store possible duplicates.
 */
 ...
 *
 * This class can be seen as a representaion of a full cards against humanity game.
 */
...
private:
    ...
    /*!
     * \brief sort indicates whether the deck shall be sorted or not.
     */
    bool sort = false;

    /*!
     * \brief sorted indicates whether the deck is currently sorted or not.
     */
    bool sorted =false;
...
public:
     ...
     *
     * Note that the added card is not copied but only a pointer to the card is stored.
     */
    void addCard(QSharedPointer<Card> card);

    /*!
     * \brief returns a list of possible duplicate card text
     * \param threshold a value betwenn 0 and 1 which indicates the maximum distance
     *        between two possible duplicated cards.
     * \return A list of possible card duplicates whitin the deck
     * \sa isPossibleDuplicate(QSharedPointer<Card> card, QSharedPointer<Card> other, double threshold)
     */
     ...
     * \param value if set to true the deck will be sorted before returning any cards.
     */
 ...
 *
 * The levenshtein-distance is used to check whether two cards are possible duplicates
 *
 ...
 * \param threshold a value between 0 and 1. A higher value means more cards will be seen as duplicates.
 *        A value of 1 will mark all pairs as duplicate. A value of 0 will mark any pair as duplicate.
